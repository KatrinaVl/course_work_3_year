...
# key_to_look_at = ['Results success:', 'Results failed:']
key_to_look_at = ['Workunits valid:', 'Workunits not completed:']
REPEATING_PER_CONFIG = 1
with open(experiment_dir / "all_result.csv", "w") as fout:
    # print("label,proj1_result_succ,proj2_result_succ,proj1_result_f,proj2_result_f", file=fout)
    # print("label,proj1_nvalid_result,proj2_nvalid_result,proj1_wrku_not_compl,proj2_wrku_not_compl", file=fout)
    ...
    for priority1 in range(1, SUM_PRIORITIES):
        ...
        config = ConfigurationFile(simulation_time = 200)
        ...
        for i in range(REPEATING_PER_CONFIG):
            ...
            for line in get_macro_stat.split('\n'):
                ...
                if (key_to_look_at[0] not in line and  key_to_look_at[1] not in line):
                    continue 
                #     Results valid:                910 (80.7%)
                # print(line)
                if key_to_look_at[0] in line:
                    # print("HERE_1")
                    # print(line)
                    # print(line.strip().split(key_to_look_at[1]))
                    get_stat = line.strip().split(key_to_look_at[0])[1].strip().split(' ')[0].replace(',', '')
                    extract_result[proj_name + "_r_s"] = get_stat
                #     Results valid:                910 (80.7%)
                # get_stat = line.strip().split('(')[1].strip().split('%)')[0].replace(',', '')
                elif key_to_look_at[1] in line:
                    # print("HERE_2")
                    # print(line)
                    # print(line.strip().split(key_to_look_at[1])[1].strip().split(' ')[0].replace(',', ''))
                    h_1 = line.strip().split(key_to_look_at[1])[1].strip().split(' ')[0].replace(',', '')
                    extract_result[proj_name + "_r_f"] = h_1
            # print(proj_name_1)
            # print(extract_result)
            print(f"{priority1}-{priority2},{extract_result[proj_name_1 + "_r_s"]},{extract_result[proj_name_2 + "_r_s"]}, {extract_result[proj_name_1 + "_r_f"]}, {extract_result[proj_name_2 + "_r_f"]}", file=fout)
